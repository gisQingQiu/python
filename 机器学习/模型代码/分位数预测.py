...
model_qrf = RandomForestQuantileRegressor(**params[3], q=params[4], random_state=42) 
model_qrf.fit(X, Y)
y_pre = model_qrf.predict(X)
lack = np.where((y_pre[0, :] >= Y.values) | (y_pre[2, :] <= Y.values), np.nan, Y.values)
x = pd.concat([X, pd.DataFrame(lack)], axis=1).dropna().iloc[:, :-1]
y = pd.Series(lack).dropna()
print(np.sum(np.isnan(lack)))

x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=params[-1])
train = dt_clean[['序号', '样点编', '样点类', '样品编', '定位经', '定位纬'] + params[1] +[name, 'geometry']]
test = dt_clean[name]
...
with rasterio.open(r"数据\环境变量\B.tif") as mode:
    mode_profile = mode.profile
    mode_profile['nodata'] = np.nan
    mode_profile['dtype'] = np.float32
    data = np.float64(mode.read(1)).reshape(-1)
    data[data==mode.nodata] = np.nan
    yamask = ~np.isnan(data)

dt = pd.DataFrame()
ya_var = pd.read_csv(r"数据\哑变量.csv")
all_of_arr = np.zeros((len(var), mode_profile['height'], mode_profile['width']))
for i, tif in enumerate(tqdm(var, desc='读取栅格')):
    if 'TRMZ' in tif or 'LUCC' in tif or 'TL' in tif or 'TS' in tif or 'YL' in tif:
        data[yamask] = np.array(ya_var[tif])
        dt[tif] = data.tolist()
    else:
...
result[result<0] = 0
with rasterio.open(f"{tifname}.tif", 'w', **mode_profile) as outfile:
